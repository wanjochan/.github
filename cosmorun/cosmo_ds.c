/*
 * cosmo_ds.c - Core Data Structures Library Implementation
 */
...
ds_map_t *ds_map_new_with(ds_hash_fn hash_fn, ds_compare_fn key_cmp) {
    ds_map_t *map = malloc(sizeof(ds_map_t));
    if (!map) return NULL;

    map->buckets = calloc(DS_MAP_DEFAULT_CAPACITY, sizeof(ds_map_entry_t *));
    if (!map->buckets) {
        free(map);
        return NULL;
    }
...
static void ds_map_resize(ds_map_t *map) {
    ...
    ds_map_entry_t **new_buckets = calloc(new_capacity, sizeof(ds_map_entry_t *));
...
void ds_map_put(ds_map_t *map, void *key, void *value) {
    ...
    /* Add new entry */
    ds_map_entry_t *new_entry = malloc(sizeof(ds_map_entry_t));
    if (!new_entry) return;
    ...
    if ((double)map->size / map->capacity > DS_MAP_LOAD_FACTOR) {
        ds_map_resize(map);
    }
}
...
ds_set_t *ds_set_new_with(ds_hash_fn hash_fn, ds_compare_fn cmp) {
    ds_set_t *set = malloc(sizeof(ds_set_t));
    if (!set) return NULL;
...
void ds_queue_enqueue(ds_queue_t *queue, void *item) {
    ...
    ds_queue_node_t *node = malloc(sizeof(ds_queue_node_t));
    if (!node) return;
...
void ds_stack_push(ds_stack_t *stack, void *item) {
    ...
    ds_stack_node_t *node = malloc(sizeof(ds_stack_node_t));
    if (!node) return;
