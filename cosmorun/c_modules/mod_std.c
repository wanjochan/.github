 ...
 * v2 System Layer: 使用 shim_* API 替换所有 libc 调用
 */
...
std_string_t* std_string_new(const char *initial) {
    std_string_t *str = (std_string_t*)shim_malloc(sizeof(std_string_t));
    if (!str) return NULL;

    size_t len = initial ? shim_strlen(initial) : 0;
    str->capacity = len + 16;
    str->data = (char*)shim_malloc(str->capacity);
    if (!str->data) {
        shim_free(str);
        return NULL;
    }
    ...
    if (initial) {
        shim_memcpy(str->data, initial, len);
    }
    str->data[len] = '\0';
    str->len = len;
...
std_string_t* std_string_with_capacity(size_t capacity) {
    ...
    str->data = (char*)shim_malloc(capacity);
    if (!str->data) {
        shim_free(str);
        return NULL;
    }
...
void std_string_free(std_string_t *str) {
    if (!str) return;
    if (str->data) shim_free(str->data);
    shim_free(str);
}
...
static void std_string_ensure_capacity(std_string_t *str, size_t required) {
    ...
    char *new_data = (char*)shim_realloc(str->data, new_capacity);
...
void std_string_append(std_string_t *str, const char *data) {
    ...
    size_t data_len = shim_strlen(data);
    std_string_ensure_capacity(str, str->len + data_len + 1);

    shim_memcpy(str->data + str->len, data, data_len);
    str->len += data_len;
    str->data[str->len] = '\0';
}
...
std_string_t* std_string_format(const char *fmt, ...) {
    ...
    shim_vsnprintf(str->data, size + 1, fmt, args);
    str->len = size;
    va_end(args);
...
std_vector_t* std_vector_with_capacity(size_t capacity) {
    std_vector_t *vec = (std_vector_t*)shim_malloc(sizeof(std_vector_t));
    if (!vec) return NULL;

    vec->data = (void**)shim_malloc(sizeof(void*) * capacity);
    if (!vec->data) {
        shim_free(vec);
        return NULL;
    }
...
void std_vector_free(std_vector_t *vec) {
    if (!vec) return;
    if (vec->data) shim_free(vec->data);
    shim_free(vec);
}
...
static void std_vector_ensure_capacity(std_vector_t *vec, size_t required) {
    ...
    void **new_data = (void**)shim_realloc(vec->data, sizeof(void*) * new_capacity);
    if (!new_data) return;
...
std_hashmap_t* std_hashmap_with_capacity(size_t capacity) {
    std_hashmap_t *map = (std_hashmap_t*)shim_malloc(sizeof(std_hashmap_t));
    if (!map) return NULL;

    map->buckets = (std_hashmap_entry_t**)shim_calloc(capacity, sizeof(std_hashmap_entry_t*));
    if (!map->buckets) {
        shim_free(map);
        return NULL;
    }
...
void std_hashmap_set(std_hashmap_t *map, const char *key, void *value) {
    ...
    while (entry) {
        if (shim_strcmp(entry->key, key) == 0) {
            entry->value = value;
            return;
        }
        entry = entry->next;
    ...
    // Create new entry
    std_hashmap_entry_t *new_entry = (std_hashmap_entry_t*)shim_malloc(sizeof(std_hashmap_entry_t));
    if (!new_entry) return;
...
std_error_t* std_error_new(int code, const char *message) {
    std_error_t *err = (std_error_t*)shim_malloc(sizeof(std_error_t));
    if (!err) return NULL;

    err->code = code;
    err->message = message ? shim_strdup(message) : NULL;
...
void std_error_free(std_error_t *err) {
    if (!err) return;
    if (err->message) shim_free(err->message);
    shim_free(err);
}
