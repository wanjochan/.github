...
int nng_listen_rep(nng_context_t *ctx, const char *url) {
    ...
    // 使用 shim_printf 替代 printf
    shim_printf("✓ REP server listening on: %s (static)\n", url);
    return NNG_OK;
}
...
int nng_dial_req(nng_context_t *ctx, const char *url) {
    ...
    // 使用 shim_printf 替代 printf
    shim_printf("✓ REQ client connected to: %s (static)\n", url);
    return NNG_OK;
}
...
int nng_bind_pub(nng_context_t *ctx, const char *url) {
    ...
    // 使用 shim_printf 替代 printf
    shim_printf("✓ PUB server bound to: %s (static)\n", url);
    return NNG_OK;
}
...
int nng_dial_sub(nng_context_t *ctx, const char *url) {
    ...
    // 使用 shim_printf 替代 printf
    shim_printf("✓ SUB client connected to: %s (static)\n", url);
    return NNG_OK;
}
...
int nng_selftest_reqrep(const char *lib_path) {
    ...
    shim_printf("✓ Static NNG implementation ready\n");
    shim_printf("Note: This is a static implementation for testing\n");
    return 0;
}
