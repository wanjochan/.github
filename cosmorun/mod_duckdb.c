// duckdb.c - DuckDB runtime module for cosmorun
// Provides reusable DuckDB helpers that can be imported via __import()
...
//// Standard library functions we rely on (declared manually for TCC minimalism)
//extern int printf(const char *format, ...);
//extern int strcmp(const char *a, const char *b);
//extern void *malloc(unsigned long size);
//extern void free(void *ptr);
//
//// Cosmopolitan dynamic loader helpers exported by cosmorun runtime
//extern void *dlopen(const char *filename, int flags);
//extern void *dlsym(void *handle, const char *symbol);
//extern int dlclose(void *handle);
//extern char *dlerror(void);

// DuckDB function pointer types
...
//static int duckdb_preferred_dlopen_flags(void) {
...
//    int flags = duckdb_preferred_dlopen_flags();
...
static int duckdb_run_query_cycle(void *handle, const char *query) {
    duckdb_open_t duckdb_open = (duckdb_open_t)dlsym(handle, "duckdb_open");
    duckdb_connect_t duckdb_connect = (duckdb_connect_t)dlsym(handle, "duckdb_connect");
    duckdb_query_t duckdb_query = (duckdb_query_t)dlsym(handle, "duckdb_query");
    duckdb_destroy_result_t duckdb_destroy_result = (duckdb_destroy_result_t)dlsym(handle, "duckdb_destroy_result");
    duckdb_disconnect_t duckdb_disconnect = (duckdb_disconnect_t)dlsym(handle, "duckdb_disconnect");
    duckdb_close_t duckdb_close = (duckdb_close_t)dlsym(handle, "duckdb_close");
    duckdb_row_count_t duckdb_row_count = (duckdb_row_count_t)dlsym(handle, "duckdb_row_count");
    duckdb_column_count_t duckdb_column_count = (duckdb_column_count_t)dlsym(handle, "duckdb_column_count");
    duckdb_column_name_t duckdb_column_name = (duckdb_column_name_t)dlsym(handle, "duckdb_column_name");
...
static int resolve_runtime(duckdb_runtime_t *rt, const char *hint) {
    ...
    rt->handle = handle;
    rt->open_fn = (duckdb_open_t)dlsym(handle, "duckdb_open");
    rt->close_fn = (duckdb_close_t)dlsym(handle, "duckdb_close");
    rt->connect_fn = (duckdb_connect_t)dlsym(handle, "duckdb_connect");
    rt->disconnect_fn = (duckdb_disconnect_t)dlsym(handle, "duckdb_disconnect");
    rt->query_fn = (duckdb_query_t)dlsym(handle, "duckdb_query");
    rt->destroy_result_fn = (duckdb_destroy_result_t)dlsym(handle, "duckdb_destroy_result");
    rt->row_count_fn = (duckdb_row_count_t)dlsym(handle, "duckdb_row_count");
    rt->column_count_fn = (duckdb_column_count_t)dlsym(handle, "duckdb_column_count");
    rt->value_varchar_fn = (duckdb_value_varchar_t)dlsym(handle, "duckdb_value_varchar");
    rt->free_fn = (duckdb_free_t)dlsym(handle, "duckdb_free");
    rt->result_error_fn = (duckdb_result_error_t)dlsym(handle, "duckdb_result_error");
...
duckdb_context_t* duckdb_init(const char *lib_path) {
    ...
    ctx->open_fn = (duckdb_open_t)dlsym(handle, "duckdb_open");
    ctx->connect_fn = (duckdb_connect_t)dlsym(handle, "duckdb_connect");
    ctx->query_fn = (duckdb_query_t)dlsym(handle, "duckdb_query");
    ctx->close_fn = (duckdb_close_t)dlsym(handle, "duckdb_close");
    ctx->disconnect_fn = (duckdb_disconnect_t)dlsym(handle, "duckdb_disconnect");
    ctx->destroy_result_fn = (duckdb_destroy_result_t)dlsym(handle, "duckdb_destroy_result");
    ctx->row_count_fn = (duckdb_row_count_t)dlsym(handle, "duckdb_row_count");
    ctx->column_count_fn = (duckdb_column_count_t)dlsym(handle, "duckdb_column_count");
    ctx->column_name_fn = (duckdb_column_name_t)dlsym(handle, "duckdb_column_name");
    ctx->value_varchar_fn = (duckdb_value_varchar_t)dlsym(handle, "duckdb_value_varchar");
    ctx->free_fn = (duckdb_free_t)dlsym(handle, "duckdb_free");
    ctx->result_error_fn = (duckdb_result_error_t)dlsym(handle, "duckdb_result_error");
