...
nng_context_t* nng_init(const char *lib_path) {
    ...
    printf("✓ NNG mock library initialized\n");
    return ctx;
}
...
int nng_listen_rep(nng_context_t *ctx, const char *url) {
    ...
    printf("✓ REP server listening on: %s (mock)\n", url);
    return NNG_OK;
}
...
int nng_dial_req(nng_context_t *ctx, const char *url) {
    ...
    printf("✓ REQ client connected to: %s (mock)\n", url);
    return NNG_OK;
}
...
int nng_bind_pub(nng_context_t *ctx, const char *url) {
    ...
    printf("✓ PUB server bound to: %s (mock)\n", url);
    return NNG_OK;
}
...
int nng_dial_sub(nng_context_t *ctx, const char *url) {
    ...
    printf("✓ SUB client connected to: %s (mock)\n", url);
    return NNG_OK;
}
...
int nng_selftest_reqrep(const char *lib_path) {
    (void)lib_path;
    printf("=== NNG REQ/REP Self Test (Mock) ===\n");
    printf("✓ Mock NNG implementation ready\n");
    printf("Note: This is a mock implementation for testing\n");
    return 0;
}

int nng_selftest_pubsub(const char *lib_path) {
    (void)lib_path;
    printf("=== NNG PUB/SUB Self Test (Mock) ===\n");
    printf("✓ Mock NNG implementation ready\n");
    printf("Note: This is a mock implementation for testing\n");
    return 0;
}