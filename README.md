```
today is the last day of past 
  and first day of future
  cherish it and make everyday count
```

wanjo 2025

## Documentation

### TGFT 统一理论框架

- [TGFT 文档索引](docs_tgft/index.md) - 统一框架索引与约定说明
- [TGFT 术语与范围提示](docs_tgft/tgft-theory.md) - 避免与主流 TGFT 混淆
- [TGFT 数学基础](docs_tgft/tgft-mathematical-foundations.md) - 微分几何与纤维丛理论
- [从 TGFT 推导广义相对论](docs_tgft/tgft-general-relativity.md) - 爱因斯坦场方程、测地线方程
- [从 TGFT 推导标准模型](docs_tgft/tgft-standard-model.md) - 电磁、弱、强相互作用
- [从 TGFT 推导量子力学](docs_tgft/tgft-quantum-mechanics.md) - 薛定谔方程、狄拉克方程
- [从 TGFT 推导统计力学](docs_tgft/tgft-statistical-mechanics.md) - 热力学与相变
- [TGFT 兼容性验证](docs_tgft/tgft-compatibility-verification.md) - 与实验数据对比

### 其他文档

- [BEC Gravity Theory](docs_tgft/becgravity.md) - 基于凝聚态量子湍流的引力引擎
